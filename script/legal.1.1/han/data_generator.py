...
# -*- encoding: utf-8 -*-
# Birth: 2022-03-01 15:34:44.069076971 +0530
# Modify: 2022-03-04 11:15:15.994678874 +0530
...
class DataGenerator:
    ...
    def fetch_gates(self, batch: list) -> torch.Tensor:
        ...
        for doc in batch:
            doc_tensor = torch.tensor([int(label in self.gates_dict[doc])
                                       for label in self.unique_labels])
    ...
    def fetch_targets(self, batch: list) -> torch.Tensor:
        ...
        for doc in batch:
            doc = torch.tensor([int(label in self.targets_dict[doc])
                                for label in self.unique_labels])
    ...
    def get_batch_embeddings(self, batch):
        ...
        """
        embedding_dim = len(list(self.embeddings.values())[0])#.size(dim=0)
